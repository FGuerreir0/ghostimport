...
import { scan, loadConfig } from './checker.js'
...
const results = await scan(targetDir, {
  ...
  scary: flags.scary,
  config,
  onProgress: flags.json || flags.quiet ? null : ({ pkg, done, total }) => {
...
if (results.hallucinated.length === 0) {
  ...
  for (const { pkg, files } of results.hallucinated) {
    ...
    if (files.length > 3) {
      console.log(`    ${c.gray(`↳ ...and ${files.length - 3} more files`)}`)
    }
  }
}
...
if (flags.scary && results.scary.length > 0) {
  ...
  if (suspicious.length > 0) {
    ...
    for (const { pkg, flags: riskFlags, downloads, created } of suspicious) {
      console.log(`  ${c.magenta('●')} ${c.bold(pkg)} ${c.gray(`(created ${created}, ${downloads ?? '?'} downloads/week)`)}`)
      for (const flag of riskFlags) {
        console.log(`    ${c.yellow('↳')} ${flag}`)
      }
    }
  }
}
